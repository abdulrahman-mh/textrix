...
const buttons = [
  {
    text: "Export JSON",
    action: () => console.log(JSON.stringify(editor.getJSON())),
  },
  {
    text: "Export HTML",
    action: () => console.log(editor.getHTML({ stripHeadlines: false })),
  },
  {
    text: "Export Metadata",
    action: () => console.log(JSON.stringify(editor.getMetadata())),
  },
  {
    text: "Export HTML (Server)",
    action: () => {
      const schema = buildSchema();
      const html = generateHTML(editor.getJSON(), schema, {
        stripHeadlines: false,
      });
      console.log(html);
    },
  },
...
buttons.forEach(({ text, action }) =>
  buttonContainer.appendChild(createButton(text, action))
);
